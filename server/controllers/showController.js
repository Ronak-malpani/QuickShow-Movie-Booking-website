...
        shows.forEach((show)=>{
    
            // -------------------- START OF FINAL ABSOLUTE FIX --------------------
            // This is the most resilient date key generation method, neutralizing local time shifts
            const storedDate = show.showDateTime; 

            // Calculate the time zone offset in minutes
            const offsetInMinutes = storedDate.getTimezoneOffset();
            
            // Create a NEW Date object shifted by the offset
            const adjustedDate = new Date(storedDate.getTime() - (offsetInMinutes * 60 * 1000));
            
            // Extract the YYYY-MM-DD format from the adjusted date
            const date = adjustedDate.toISOString().split("T")[0]; 

            // -------------------- END OF FINAL ABSOLUTE FIX ----------------------
...
            }
            ...
            console.log(`Processing Show: Key=${date} Time=${storedDate.toTimeString().split(' ')[0].slice(0,5)}`);
            
            // Push an object containing both time and showId for the frontend
            dateTime[date].push({time:storedDate.toTimeString().split(' ')[0].slice(0,5),showId:show._id})
        })
        
